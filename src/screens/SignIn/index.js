    ...
    const handleSignClick =  () => {
        ...
        if(emailField != '' && passwordField.length > 5){
            Api.login(emailField, passwordField).then((snapshot)=>{
                Api.addAuthListener((userr)=>{
                    if(userr){
                        Api.getUsuario(userr.uid).then((snapsho)=>{ 
                            userDispatch({
                                type: 'setAvatar',
                                payload:{
                                    avatar: snapsho.val().avatar
                                }
                            });
                        });
                        AsyncStorage.setItem('tokenUser', JSON.stringify(userr));
                        setLoading(false)
                        setLoadingNot(true)
                        if(user.logar.key) {
                            navigation.navigate('Perfil',  {
                                key:user.logar.key,
                                nome: user.logar.nome,
                                estrela: user.logar.estrela,
                                avatar: user.logar.avatar
                            })
                        }else{
                            navigation.reset({
                                routes:[{name: 'Preload'}]
